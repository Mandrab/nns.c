...
START_TEST(test_random_distribution_mean)
{
    float samples[SAMPLES_COUNT], accumulator = 0;
    ...
    // int discrete_samples[100] = { 0 };
    // discretize_samples(samples, SAMPLES_COUNT, discrete_samples, 100);

    // check that the average is the mean of the distribution
    ck_assert_msg(
        -ACCURACY <= accumulator && accumulator <= ACCURACY,
    ...
    ck_assert_msg(
        ...
        "The mean of the distribution is %f instead of 100", accumulator
    );
}
END_TEST

START_TEST(test_random_distribution_shape)
{
    float samples[SAMPLES_COUNT], min = FLT_MAX, max = FLT_MIN;
    ...
    for (int i = 0; i < QUANTS_COUNT - 1; i++)
    {
        ...
        ck_assert_msg(
            test,
            "The distribution is not normal: #%d = %d, #%d = %d",
...
Suite* distributions_suite()
{
    Suite* suite = suite_create("Distributions");
    TCase* tc_core = tcase_create("Random distribution");

    tcase_add_test(tc_core, test_random_distribution_mean);
    tcase_add_test(tc_core, test_random_distribution_shape);
